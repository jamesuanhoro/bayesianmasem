...
#' A function to impute a correlation matrix with missing elements
#' @param s_mat (matrix) Correlation or covariance matrix
#' @param n (integer) Sample size
#' @param correlation (LOGICAL) If TRUE, assume correlation matrix;
#' If FALSE, assume covariance matrix.
#' @param n Sample size
#' @returns Returns a list with the lower half vector of log(correlation matrix)
#' and its asymptotic variance matrix
#' @keywords internal
.get_log_mat_missing <- function(
    s_mat, n = 1, correlation = TRUE, n_imps = 2e3) {

  if (!isFALSE(correlation)) {
    s_mat <- suppressWarnings(stats::cov2cor(s_mat))
  }

  reject <- TRUE

  while (reject) {
    s_c <- fungible::CompleteRmap(s_mat, n_imps, MinLambda = 1e-3)$Ri
    loc_s <- do.call("rbind", lapply(s_c, \(x) g_map(x[lower.tri(x)])))
    cov_s <- lapply(s_c, \(x) get_avar_mat(x, n))
    keep_results <- unlist(lapply(cov_s, \(x) sum(is.na(x)))) == 0

    loc_s <- loc_s[keep_results, , drop = FALSE]
    cov_s <- cov_s[keep_results]

    n_valid <- sum(keep_results)
    var_w <- Reduce("+", cov_s) / n_valid
    var_b <- cov(loc_s)
    var_total <- var_w + (1 + 1 / n_valid) * var_b

    if (min(eigen(var_total)$values) > 0 && n_valid > n_imps %/% 2 + 1) {
      reject <- FALSE
    }
  }

  list(u_bar = colMeans(loc_s), u_var = var_total)
}
