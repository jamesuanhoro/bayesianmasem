...
#' @inheritParams .create_data_list_pooled
#' @returns Data list object used in fitting Stan model
#' @keywords internal
.create_data_list_pa_pooled <- function(
    lavaan_object = NULL,
    method = "normal",
    priors = NULL,
    partab = NULL,
  ...
  data_list$r_mat_s <- array(dim = c(1, data_list$Ni, data_list$Ni))
  data_list$r_mat_s[1, , ] <- data_list$S
  r_vec <- data_list$S[lower.tri(data_list$S, diag = FALSE)]
  ni_sq <- length(r_vec)
  data_list$r_obs_vec_inp <- array(g_map(r_vec), dim = c(1, ni_sq))
  data_list$r_obs_vec_cov_inp <- array(dim = c(1, ni_sq, ni_sq))
  tmp_acov <- acov_mat
  ...
  data_list$r_obs_vec_cov_inp[1, , ] <- get_avar_mat(
