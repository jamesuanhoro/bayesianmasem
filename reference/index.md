# Package index

## All functions

- [`Norton13`](https://jamesuanhoro.github.io/bayesianmasem/reference/Norton13.md)
  : Studies on the Hospital Anxiety and Depression Scale.
- [`bayesianmasem`](https://jamesuanhoro.github.io/bayesianmasem/reference/bayesianmasem.md)
  [`bayesianmasem-package`](https://jamesuanhoro.github.io/bayesianmasem/reference/bayesianmasem.md)
  : The 'bayesianmasem' package.
- [`bmasem-class`](https://jamesuanhoro.github.io/bayesianmasem/reference/bmasem-class.md)
  : A class for models fitted with the package.
- [`bmasem()`](https://jamesuanhoro.github.io/bayesianmasem/reference/bmasem.md)
  : Fit random-effects Bayesian meta-analytic CFAs with minor factors
  assumed.
- [`bmasem_composite_reliability()`](https://jamesuanhoro.github.io/bayesianmasem/reference/bmasem_composite_reliability.md)
  : Obtain composite reliability from a fitted model
- [`bmasem_plot()`](https://jamesuanhoro.github.io/bayesianmasem/reference/bmasem_plot.md)
  : Plot model results
- [`bmasem_residual_network()`](https://jamesuanhoro.github.io/bayesianmasem/reference/bmasem_residual_network.md)
  : Compute residual network
- [`bmasem_stage_1()`](https://jamesuanhoro.github.io/bayesianmasem/reference/bmasem_stage_1.md)
  : Fit random-effects Bayesian meta-analytic CFAs with minor factors
  assumed.
- [`bmasem_stage_2()`](https://jamesuanhoro.github.io/bayesianmasem/reference/bmasem_stage_2.md)
  : Fit random-effects Bayesian meta-analytic CFAs with minor factors
  assumed.
- [`bmasempriors-class`](https://jamesuanhoro.github.io/bayesianmasem/reference/bmasempriors-class.md)
  : A class for setting up priors.
- [`g_inv_map()`](https://jamesuanhoro.github.io/bayesianmasem/reference/g_inv_map.md)
  : Transform unbounded vector to correlation matrix
- [`g_map()`](https://jamesuanhoro.github.io/bayesianmasem/reference/g_map.md)
  : Log matrix function using eigendecomposition
- [`get_asy_cov()`](https://jamesuanhoro.github.io/bayesianmasem/reference/get_asy_cov.md)
  : Asymptotic variance matrix of lower half vector of correlation
  matrix
- [`get_avar_mat()`](https://jamesuanhoro.github.io/bayesianmasem/reference/get_avar_mat.md)
  : Asymptotic variance matrix of lower half vector of log(correlation
  matrix)
- [`issp89`](https://jamesuanhoro.github.io/bayesianmasem/reference/issp89.md)
  : Another dataset from the metaSEM package.
- [`log_m()`](https://jamesuanhoro.github.io/bayesianmasem/reference/log_m.md)
  : Log matrix function using eigendecomposition
- [`new_bmasempriors()`](https://jamesuanhoro.github.io/bayesianmasem/reference/new_bmasempriors.md)
  : Set priors in package
- [`pp_summary()`](https://jamesuanhoro.github.io/bayesianmasem/reference/pp_summary.md)
  : Pretty print model results
